...
| **kernel** | `KernelPeerCarrier` | **taught:** in-process `cek_peer_pyo3` (`PeerAbi` construct → bind → apply → release). Same JSON as wasm. Not a Python Peer kernel. |
...
**Leftover (untaught):** subprocess `cek apply` via `CEK_KERNEL_CARRIER=subprocess` or `open_carrier("kernel", backend="subprocess")`. Named leftover until absence lock — not a second taught default.

Install the taught module from [cek-runtime](https://github.com/bitplorer/cek-runtime) (`ports/cek-peer-pyo3`): `cargo build -p cek-peer-pyo3 --features extension-module --release`, then `export CEK_PEER_PYO3=$PWD/target/release/libcek_peer_pyo3.so`.
