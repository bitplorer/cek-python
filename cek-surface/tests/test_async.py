...
def test_async_handler_asubmit():
    s = _surface()
    ...
    cap = s.mint("hello")
    r = asyncio.run(s.asubmit("hello", {}, cap=cap, drain_async=False))
    assert r["result"]["kind"] == "ok"
...
def test_sync_submit_refuses_async_handler():
    ...
    try:
        ...
        assert "asubmit" in str(e)


def test_asubmit_runs_sync_handler():
    s = _surface()
...
def test_host_asubmit_same_law():
