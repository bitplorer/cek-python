...
| 2 | **verify before compose** | Surface `check()` before handler; BoundAsk is Phase 2 | `test_host_kernel` + Surface path |
...
Unclaimed (Phase 2 — do not list in SHIPPED as held): BoundAsk ctor, IdemBackend, lineage reverse, Ed25519, law-generation window, concurrent-once property.
